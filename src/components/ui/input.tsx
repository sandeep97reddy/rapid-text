...
import { useIsClickThroughActive } from "@/hooks/useClickThrough";

function Input({ className, type, disabled, tabIndex, ...props }: React.ComponentProps<"input">) {
  const isClickThrough = useIsClickThroughActive();

  return (
    <input
      type={type}
      data-slot="input"
      disabled={disabled || isClickThrough}
      tabIndex={isClickThrough ? -1 : tabIndex}
      className={cn(
