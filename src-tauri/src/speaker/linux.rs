// Rapid Text linux speaker input and stream
...
pub fn get_input_devices() -> Result<Vec<AudioDevice>> {
    ...
    let mut context = Context::new(&mainloop, "rapid-text-device-enum")
...
impl SpeakerInput {
    ...
    pub fn stream(self, active_streams_count: Arc<std::sync::atomic::AtomicUsize>) -> Result<SpeakerStream> {
        ...
        if !init_success {
            ...
            return Err(anyhow::anyhow!("Linux audio capture failed to initialize"));
        }
...
impl Drop for SpeakerStream {
    fn drop(&mut self) {
        ...
        self.active_streams_count.fetch_sub(1, std::sync::atomic::Ordering::SeqCst);
    }
}
