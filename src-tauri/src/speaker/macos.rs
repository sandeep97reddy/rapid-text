// Rapid Text macos speaker input and stream
...
impl SpeakerInput {
    ...
    pub fn stream(self, active_streams_count: Arc<std::sync::atomic::AtomicUsize>) -> Result<SpeakerStream> {
        let asbd = self.tap.asbd().map_err(|e| anyhow::anyhow!("Failed to get ASBD: {:?}", e))?;
        ...
        active_streams_count.fetch_add(1, Ordering::SeqCst);
...
impl Drop for SpeakerStream {
    fn drop(&mut self) {
        ...
        self.active_streams_count.fetch_sub(1, Ordering::SeqCst);
    }
}
