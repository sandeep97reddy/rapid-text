...
// Re-export commands and dsp for tauri handler
pub use commands::*;
pub use dsp::*;
...
// Rapid Text speaker input and stream
...
impl SpeakerInput {
    ...
    pub fn new() -> Result<Self> {
        Self::new_with_device(None, false)
    }

    // Creates a new speaker input with a specific device ID
    #[cfg(any(target_os = "macos", target_os = "windows", target_os = "linux"))]
    pub fn new_with_device(device_id: Option<String>, is_mic: bool) -> Result<Self> {
        #[cfg(target_os = "windows")]
        let inner = PlatformSpeakerInput::new(device_id, is_mic)?;
    ...
    #[cfg(not(any(target_os = "macos", target_os = "windows", target_os = "linux")))]
    pub fn new_with_device(_device_id: Option<String>, _is_mic: bool) -> Result<Self> {
        Err(anyhow::anyhow!(
    ...
    #[cfg(any(target_os = "macos", target_os = "windows", target_os = "linux"))]
    pub fn stream(self, active_streams_count: std::sync::Arc<std::sync::atomic::AtomicUsize>) -> Result<SpeakerStream> {
        let inner = self.inner.stream(active_streams_count)?;
        Ok(SpeakerStream { inner })
    }

    #[cfg(not(any(target_os = "macos", target_os = "windows", target_os = "linux")))]
    pub fn stream(self, _active_streams_count: std::sync::Arc<std::sync::atomic::AtomicUsize>) -> Result<SpeakerStream> {
